			...
			if(fieldType != null) {
				...
				if(docField instanceof StructuredTextField) {
					...
					if(fieldType.textual) {
						docBuilder.field("stemmed_" + fieldName, textField.getValue());
						docBuilder.field("literal_" + fieldName, textField.getValue());
						docBuilder.field("case_sensitive_" + fieldName, textField.getValue());
					...
					if(fieldType.hasParagraphs) {
						...
						XContentBuilder paragraphsBuilder = XContentFactory.jsonBuilder().startArray();
						for(String paragraph : paragraphs) {
							paragraphsBuilder.startObject().field("paragraph", paragraph).endObject();
						}
						paragraphsBuilder.endArray();
						
						docBuilder.field(fieldName + "_paragraphs", paragraphsBuilder);
						docBuilder.field("stemmed_" + fieldName + "_paragraphs", paragraphsBuilder);
						docBuilder.field("literal_" + fieldName + "_paragraphs", paragraphsBuilder);
						docBuilder.field("case_sensitive_" + fieldName + "_paragraphs", paragraphsBuilder);
						
						XContentBuilder sentencesBuilder = XContentFactory.jsonBuilder().startArray();
						for(String paragraph : paragraphs) {
							List<Sentence> sentences = TextUtils.getSentences(paragraph);
							for(Sentence sentence : sentences) {
								sentencesBuilder.startObject().field("sentence", sentence.getText()).endObject();
							}
						}
						sentencesBuilder.endArray();
						
						docBuilder.field(fieldName + "_sentences", sentencesBuilder);
						docBuilder.field("stemmed_" + fieldName + "_sentences", sentencesBuilder);
						docBuilder.field("literal_" + fieldName + "_sentences", sentencesBuilder);
						docBuilder.field("case_sensitive_" + fieldName + "_sentences", sentencesBuilder);
					}
					else {
